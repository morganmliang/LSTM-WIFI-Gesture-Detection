...
def train_model(model: Model, dataset_id, dataset_prefix, epochs=50, batch_size=128, val_subset=None,
                cutoff=None, normalize_timeseries=False, learning_rate=1e-5):
    """
    ...
    class_weight = cw.compute_class_weight('balanced',classes,y_ind)
    class_weight = {i : class_weight[i] for i in range(len(le.classes_))}
...
def evaluate_model(model: Model, dataset_id, dataset_prefix, batch_size=128, test_data_subset=None,
    ...
    print(predictions)
    mat = confusion_matrix(y_test.argmax(axis=1), predictions)
    print(mat)
    labels_new = ['vertical hand', 'wave up down', 'push pull']
    new_mat = ConfusionMatrixDisplay(mat, display_labels=labels_new)

    print(classification_report(y_test.argmax(axis=1), predictions, target_names=labels_new))

    new_mat.plot()
    new_mat.ax_.set(
                title='Confusion Matrix - Split 1', 
                xlabel='Predicted', 
                ylabel='True')
    txt = dataset_prefix.split('/')[0]
    plt.savefig('confusion_matrix_{}.jpg'.format(txt))
